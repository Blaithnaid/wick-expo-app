...
export default function TikTokFiltersPage() {
	...
	const [filterVideos, setFilterVideos] =
		useState<FilterWithVideo[]>(filters);
					...
					filters.map(async (filter): Promise<FilterWithVideo> => {
						if (filter.videoPath) {
							try {
								...
								console.log(
									`Successfully got URL for ${filter.title}: ${videoUrl}`
								);
								return { ...filter, videoUrl };
							} catch (error) {
								console.error(
									`Error getting video URL for ${filter.title}:`,
									error
								);
		...
		>
			...
			<View className="max-w-5xl mx-auto">
				...
				<Text className="text-sm text-center mb-6 text-gray-700 dark:text-gray-300">
					Note: Make sure you have the TikTok app installed to use
					these filters.
				</Text>
						...
						{pair.map((filter, colIndex) => (
							...
							>
								...
								<View className="flex-row justify-between items-center">
									<Pressable
										onPress={() =>
											Linking.openURL(filter.tiktokLink)
										}
				...
				>
					<View className="bg-white dark:bg-gray-800 rounded-xl p-6 w-80 items-center">
						...
						<Text className="text-sm text-center mb-4 text-gray-700 dark:text-gray-300">
							Scan this QR code with your phone to open the filter
							in TikTok
						</Text>
